(function(testCase){
  ...
  console.debug('Testing Immutable IndexedSeq with 1000 Float elements');
  console.debug('*****************************************************');

  var data_01 = Immutable.fromJS(d3.range(0, 1000).map(Math.random));
  var data_02 = Immutable.fromJS(d3.range(500, 1000).map(Math.random));
  var data_03 = Immutable.fromJS(d3.range(10, 100).map(Math.random));
